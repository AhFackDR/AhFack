-- AhFack Rotations, Rogue Assassination -- PvE
...
local offvanish = dark_addon.settings.fetch('dr_example_offvanish')
local shadow = dark_addon.settings.fetch('dr_example_shadow.check')
local shadowpercent = dark_addon.settings.fetch('dr_example_shadow.spin', 25)
...
local vialpercent = dark_addon.settings.fetch('dr_example_vial.spin', 45)
...
--print (y) 
    ...
    if modifier.shift and player.spell(SB.Blind).cooldown == 0 and target.distance < 15 then 
      return cast(SB.Blind, 'target')
    end
...
if talent(6,2) then
	  ...
	  if player.buff(SB.Stealth).up and player.spell(SB.CheapShot).cooldown == 0 and player.power.energy.actual >= 45 then 
      return cast(SB.CheapShot)
    end
   ...
   if player.buff(SB.Subterfuge).down and -spell(SB.Vanish) == 0 and player.power.energy.actual >= 45 and -spell(SB.Garrote) == 0 and target.debuff(SB.Garrote).remains <= 5.4 and offvanish == true then 
   	return cast(SB.Vanish)
   end
...
--print (y)
 	...
 	if lastcast(SB.Vanish) and -spell(SB.CheapShot) == 0 and player.power.energy.actual >= 45 and offvanish == true then
...
local function interface()
   local example = {
    key = 'dr_example',
    title =                     'AhFack Rotations - Rogue PvP Assassination',
    width = 250,
    height = 320,
    resize = true,
    show = false,
    template = {
      ...
      { key = 'offvanish', type = 'checkbox', text = 'Offensive Vanish', desc = 'Use Vanish Offensively' },
      ...
      { key = 'vial', type = 'checkspin', text = 'Vial of Crimson HP%', desc = 'What % you will be using Vial of Crimson at', default_spin = 45, min = 10, max = 100, step = 5 },
